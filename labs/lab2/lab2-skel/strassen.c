...
int main(int argc, char *argv[])
{
	...
	r = pthread_create(&threads[0], NULL, calM1, &arguments[0]);
	r = pthread_create(&threads[1], NULL, calM2, &arguments[1]);
	r = pthread_create(&threads[2], NULL, calM3, &arguments[2]);
	r = pthread_create(&threads[3], NULL, calM4, &arguments[3]);
	r = pthread_create(&threads[4], NULL, calM5, &arguments[4]);
	r = pthread_create(&threads[5], NULL, calM6, &arguments[5]);
	r = pthread_create(&threads[6], NULL, calM7, &arguments[6]);
	...
	r = pthread_create(&threads[0], NULL, calC11, &arguments[0]);
	r = pthread_create(&threads[1], NULL, calC12, &arguments[1]);
	r = pthread_create(&threads[2], NULL, calC21, &arguments[2]);
	r = pthread_create(&threads[3], NULL, calC22, &arguments[3]);
	for (i = 0; i < 4; i++) {
		r = pthread_join(threads[i], &status);
