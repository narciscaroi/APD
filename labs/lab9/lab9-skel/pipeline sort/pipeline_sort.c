...
int main(int argc, char * argv[]) {
	...
	if (rank == 0) { // This code is run by a single process
		...
		//recv val initiala
		MPI_Recv(&number, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, NULL); 
		...
		for (int i = 0; i < nProcesses - rank; i++) {
			...
			if(number == -1) {
				...
				//atunci salvăm noua valoare și o trimitem pe cea veche procesului următor
				MPI_Send(&number, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD); 
				number = recv;
			} 
			else if (recv >= number) {
				//atunci o trimitem direct procesului următor
